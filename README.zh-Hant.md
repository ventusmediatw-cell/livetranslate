...
> **關於計費 / 方案。** 本 App 用的是 Gemini 即時模型 `gemini-3.5-live-translate-preview`。免費 key 可能可用，但這個預覽模型有時需要在你的 Google Cloud 專案**開啟計費**。若 App 卡在「連線中」或翻不出來，到 Google Cloud 為專案開啟計費再試。
